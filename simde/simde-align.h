...
#elif (defined(__cplusplus) && (__cplusplus >= 201103L)) || (0 && HEDLEY_HAS_FEATURE(cxx_alignof))
#define SIMDE_ALIGN_OF(Type) alignof(Type)
#elif HEDLEY_GCC_VERSION_CHECK(2, 95, 0) || HEDLEY_ARM_VERSION_CHECK(4, 1, 0) ||                   \
    HEDLEY_INTEL_VERSION_CHECK(13, 0, 0) || HEDLEY_SUNPRO_VERSION_CHECK(5, 13, 0) ||               \
    HEDLEY_TINYC_VERSION_CHECK(0, 9, 24) || HEDLEY_PGI_VERSION_CHECK(19, 10, 0) ||                 \
    HEDLEY_CRAY_VERSION_CHECK(10, 0, 0) || HEDLEY_TI_ARMCL_VERSION_CHECK(16, 9, 0) ||              \
    HEDLEY_TI_CL2000_VERSION_CHECK(16, 9, 0) || HEDLEY_TI_CL6X_VERSION_CHECK(8, 0, 0) ||           \
    HEDLEY_TI_CL7X_VERSION_CHECK(1, 2, 0) || HEDLEY_TI_CL430_VERSION_CHECK(16, 9, 0) ||            \
    HEDLEY_TI_CLPRU_VERSION_CHECK(2, 3, 2) || HEDLEY_MCST_LCC_VERSION_CHECK(1, 25, 10) ||          \
...
#define SIMDE_ALIGN_CAP(Alignment)                                                                 \
    (((Alignment) < (SIMDE_ALIGN_PLATFORM_MAXIMUM)) ? (Alignment) : (SIMDE_ALIGN_PLATFORM_MAXIMUM))
#else
 ...
 */
    ...
    HEDLEY_CRAY_VERSION_CHECK(8, 4, 0) || HEDLEY_IBM_VERSION_CHECK(11, 1, 0) ||                    \
    HEDLEY_INTEL_VERSION_CHECK(13, 0, 0) || HEDLEY_PGI_VERSION_CHECK(19, 4, 0) ||                  \
    HEDLEY_ARM_VERSION_CHECK(4, 1, 0) || HEDLEY_TINYC_VERSION_CHECK(0, 9, 24) ||                   \
    HEDLEY_TI_ARMCL_VERSION_CHECK(16, 9, 0) || HEDLEY_TI_CL2000_VERSION_CHECK(16, 9, 0) ||         \
    HEDLEY_TI_CL6X_VERSION_CHECK(8, 0, 0) || HEDLEY_TI_CL7X_VERSION_CHECK(1, 2, 0) ||              \
    HEDLEY_TI_CL430_VERSION_CHECK(16, 9, 0) || HEDLEY_TI_CLPRU_VERSION_CHECK(2, 3, 2)
 ...
 */
#if HEDLEY_HAS_BUILTIN(__builtin_assume_aligned) || HEDLEY_GCC_VERSION_CHECK(4, 7, 0)
...
#if defined(__cplusplus)
template<typename T>
HEDLEY_ALWAYS_INLINE static T *simde_align_assume_to_unchecked(T *ptr, const size_t alignment)
#else
...
#if defined(__cplusplus)
template<typename T>
static HEDLEY_ALWAYS_INLINE T *simde_align_assume_to_checked_uncapped(T *ptr,
                                                                      const size_t alignment,
                                                                      const char *file,
                                                                      int line,
                                                                      const char *ptrname)
#else
static HEDLEY_ALWAYS_INLINE void *simde_align_assume_to_checked_uncapped(void *ptr,
                                                                         const size_t alignment,
                                                                         const char *file,
                                                                         int line,
                                                                         const char *ptrname)
#endif
{
    if (HEDLEY_UNLIKELY(
            (HEDLEY_REINTERPRET_CAST(SIMDE_ALIGN_INTPTR_T_, (ptr)) %
             HEDLEY_STATIC_CAST(SIMDE_ALIGN_INTPTR_T_, SIMDE_ALIGN_CAP(alignment))) != 0)) {
        fprintf(stderr,
                ...
                HEDLEY_STATIC_CAST(
                    unsigned int,
                    HEDLEY_REINTERPRET_CAST(SIMDE_ALIGN_INTPTR_T_, (ptr)) %
                        HEDLEY_STATIC_CAST(SIMDE_ALIGN_INTPTR_T_, SIMDE_ALIGN_CAP(alignment))));
    }
...
#if defined(__cplusplus)
#define SIMDE_ALIGN_ASSUME_TO(Pointer, Alignment)                                                  \
    simde_align_assume_to_checked_uncapped((Pointer), (Alignment), __FILE__, __LINE__, #Pointer)
#else
#define SIMDE_ALIGN_ASSUME_TO(Pointer, Alignment)                                                  \
