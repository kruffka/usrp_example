...
static void trx_usrp_start_interdigital_gpio(hw_device *device, usrp_state_t *s)
{
...
static void trx_usrp_start_generic_gpio(hw_device *device, usrp_state_t *s)
{
...
static int trx_usrp_start(hw_device *device) {
  ...
  switch (device->hw_cfg->gpio_controller) {
...
static int trx_usrp_write(hw_device *device,
  ...
  int ret=0;
  usrp_state_t *s = (usrp_state_t *)device->priv;
  timestamp -= device->hw_cfg->command_line_sample_advance + device->hw_cfg->tx_sample_advance;
...
*/
static int trx_usrp_read(hw_device *device, hw_timestamp *ptimestamp, void **buff, int nsamps, int cc) {
  usrp_state_t *s = (usrp_state_t *)device->priv;
  ...
  *ptimestamp = s->rx_timestamp;
  ...
  if (device->hw_cfg->recplay_mode == RECPLAY_RECORDMODE) { // record mode
    ...
    if (recPlay->nbSamplesBlocks <= device->hw_cfg->recplay_conf->u_sf_max &&
...
void *freq_thread(void *arg) {
  hw_device *device=(hw_device *)arg;
  usrp_state_t *s = (usrp_state_t *)device->priv;
  uhd::tune_request_t tx_tune_req(device->hw_cfg[0].tx_freq[0],
                                  device->hw_cfg[0].tune_offset);
  uhd::tune_request_t rx_tune_req(device->hw_cfg[0].rx_freq[0],
                                  device->hw_cfg[0].tune_offset);
 ...
 * \param device the hardware to use
 * \param hw_cfg RF frontend parameters set by application
 * \param dummy dummy variable not used
 * \returns 0 in success
 */
int trx_usrp_set_freq(hw_device *device, openair0_config_t *hw_cfg)
{
  usrp_state_t *s = (usrp_state_t *)device->priv;
  printf("Setting USRP TX Freq %f, RX Freq %f, tune_offset: %f\n", hw_cfg[0].tx_freq[0], hw_cfg[0].rx_freq[0], hw_cfg[0].tune_offset);

  uhd::tune_request_t tx_tune_req(hw_cfg[0].tx_freq[0], hw_cfg[0].tune_offset);
  uhd::tune_request_t rx_tune_req(hw_cfg[0].rx_freq[0], hw_cfg[0].tune_offset);
  s->usrp->set_tx_freq(tx_tune_req);
  s->usrp->set_rx_freq(rx_tune_req);
 ...
 * \returns 0 in success
 */
int openair0_set_rx_frequencies(hw_device *device, openair0_config_t *hw_cfg) {
  usrp_state_t *s = (usrp_state_t *)device->priv;
  uhd::tune_request_t rx_tune_req(hw_cfg[0].rx_freq[0], hw_cfg[0].tune_offset);
  printf("In openair0_set_rx_frequencies, freq: %f, tune offset: %f\n",
         hw_cfg[0].rx_freq[0],  hw_cfg[0].tune_offset);
  //rx_tune_req.rf_freq_policy = uhd::tune_request_t::POLICY_MANUAL;
  //rx_tune_req.rf_freq = hw_cfg[0].rx_freq[0];
  s->usrp->set_rx_freq(rx_tune_req);
  return(0);
}
 ...
 * \returns 0 in success
 */
int trx_usrp_set_gains(hw_device *device,
                       openair0_config_t *hw_cfg) {
  usrp_state_t *s = (usrp_state_t *)device->priv;
  ::uhd::gain_range_t gain_range_tx = s->usrp->get_tx_gain_range(0);
  s->usrp->set_tx_gain(gain_range_tx.stop()-hw_cfg[0].tx_gain[0]);
  ::uhd::gain_range_t gain_range = s->usrp->get_rx_gain_range(0);

  // limit to maximum gain
  if (hw_cfg[0].rx_gain[0]-hw_cfg[0].rx_gain_offset[0] > gain_range.stop()) {
    LOG_E(HW,"RX Gain 0 too high, reduce by %f dB\n",
          hw_cfg[0].rx_gain[0]-hw_cfg[0].rx_gain_offset[0] - gain_range.stop());
    int gain_diff = gain_range.stop() - (hw_cfg[0].rx_gain[0] - hw_cfg[0].rx_gain_offset[0]);
    return gain_diff;
  }

  s->usrp->set_rx_gain(hw_cfg[0].rx_gain[0]-hw_cfg[0].rx_gain_offset[0]);
  LOG_I(HW,"Setting USRP RX gain to %f (rx_gain %f,gain_range.stop() %f)\n",
        hw_cfg[0].rx_gain[0]-hw_cfg[0].rx_gain_offset[0],
        hw_cfg[0].rx_gain[0],gain_range.stop());
  return(0);
}
 ...
 */
int trx_usrp_stop(hw_device *device) {
  return(0);
}
 ...
 * \returns 0 in success
 */
void set_rx_gain_offset(openair0_config_t *hw_cfg, int chain_index,int bw_gain_adjust) {
  int i=0;
  // loop through calibration table to find best adjustment factor for RX frequency
  double min_diff = 6e9,diff,gain_adj=0.0;

  if (bw_gain_adjust==1) {
    switch ((int)hw_cfg[0].sample_rate) {
      case 46080000:
        break;
      ...
      default:
        LOG_E(HW,"unknown sampling rate %d\n",(int)hw_cfg[0].sample_rate);
        //exit(-1);
        break;
    }
  }

  while (hw_cfg->rx_gain_calib_table[i].freq>0) {
    diff = fabs(hw_cfg->rx_freq[chain_index] - hw_cfg->rx_gain_calib_table[i].freq);
    LOG_I(HW,"cal %d: freq %f, offset %f, diff %f\n",
          i,
          hw_cfg->rx_gain_calib_table[i].freq,
          hw_cfg->rx_gain_calib_table[i].offset,diff);

    if (min_diff > diff) {
      min_diff = diff;
      hw_cfg->rx_gain_offset[chain_index] = hw_cfg->rx_gain_calib_table[i].offset+gain_adj;
    }
...
*/
int trx_usrp_get_stats(hw_device *device) {
  return(0);
}
 ...
 * \returns  0 on success
 */
int trx_usrp_reset_stats(hw_device *device) {
  return(0);
}
...
extern "C" {
  int device_init(hw_device *device, hw_config_t *hw_cfg) {
    LOG_I(HW, "hw_cfg[0].sdr_addrs == '%s'\n", hw_cfg[0].sdr_addrs);
    LOG_I(HW, "hw_cfg[0].clock_source == '%d' (internal = %d, external = %d)\n", hw_cfg[0].clock_source,internal,external);
    usrp_state_t *s ;
    int choffset = 0;
    ...
    if (device->hw_cfg->recplay_mode == RECPLAY_RECORDMODE) {
    ...
    if (hw_cfg[0].sdr_addrs == NULL) {
      args = "type=b200";
    } else {
      args = hw_cfg[0].sdr_addrs;
	...
	if (hw_cfg[0].clock_source == internal) {
	  s->usrp->set_clock_source("internal");
	  LOG_I(HW,"Setting clock source to internal\n");
	}
	else if (hw_cfg[0].clock_source == external ) {
	  s->usrp->set_clock_source("external");
	  LOG_I(HW,"Setting clock source to external\n");
	}
	else if (hw_cfg[0].clock_source==gpsdo) {
	...
	if (hw_cfg[0].clock_source != unset) {
  ...
  if (device->type==USRP_N300_DEV || device->type==USRP_X300_DEV || device->type==USRP_X400_DEV) {
    LOG_I(HW,"%s() sample_rate:%u\n", __FUNCTION__, (int)hw_cfg[0].sample_rate);

    switch ((int)hw_cfg[0].sample_rate) {
      case 245760000:
        // from usrp_time_offset
        // hw_cfg[0].samples_per_packet    = 2048;
        hw_cfg[0].tx_sample_advance = 15; // to be checked
        hw_cfg[0].tx_bw = 200e6;
        hw_cfg[0].rx_bw = 200e6;
        break;
      ...
      case 184320000:
        ...
        // hw_cfg[0].samples_per_packet    = 2048;
        hw_cfg[0].tx_sample_advance = 15; // to be checked
        hw_cfg[0].tx_bw = 100e6;
        hw_cfg[0].rx_bw = 100e6;
        break;
      ...
      case 1920000:
        //hw_cfg[0].samples_per_packet    = 2048;
        hw_cfg[0].tx_sample_advance     = 50;
        hw_cfg[0].tx_bw                 = 1.25e6;
        hw_cfg[0].rx_bw                 = 1.25e6;
        break;

      default:
        LOG_E(HW,"Error: unknown sampling rate %f\n",hw_cfg[0].sample_rate);
        exit(-1);
        break;
    }
  }
  ...
  if (device->type == USRP_B200_DEV) {
    if ((vers == 3) && (subvers == 9) && (subsubvers>=2)) {
      hw_cfg[0].rx_gain_calib_table = calib_table_b210;
      bw_gain_adjust=0;
      std::cerr << "-- Using calibration table: calib_table_b210" << std::endl; // Bell Labs info
    } else {
      hw_cfg[0].rx_gain_calib_table = calib_table_b210_38;
      bw_gain_adjust=1;
    ...
    switch ((int)hw_cfg[0].sample_rate) {
      case 46080000:
        ...
        //hw_cfg[0].samples_per_packet    = 1024;
        hw_cfg[0].tx_sample_advance     = 164;
        hw_cfg[0].tx_bw                 = 40e6;
        hw_cfg[0].rx_bw                 = 40e6;
        break;
      ...
      case 30720000:
        ...
        //hw_cfg[0].samples_per_packet    = 1024;
        hw_cfg[0].tx_sample_advance     = 115;
        hw_cfg[0].tx_bw                 = 20e6;
        hw_cfg[0].rx_bw                 = 20e6;
        break;
      ...
      case 23040000:
        ...
        //hw_cfg[0].samples_per_packet    = 1024;
        hw_cfg[0].tx_sample_advance     = 113;
        hw_cfg[0].tx_bw                 = 20e6;
        hw_cfg[0].rx_bw                 = 20e6;
        break;
      ...
      case 15360000:
        ...
        //hw_cfg[0].samples_per_packet    = 1024;
        hw_cfg[0].tx_sample_advance     = 103;
        hw_cfg[0].tx_bw                 = 20e6;
        hw_cfg[0].rx_bw                 = 20e6;
        break;
  ...
  /* device specific */
  //hw_cfg[0].txlaunch_wait = 1;//manage when TX processing is triggered
  //hw_cfg[0].txlaunch_wait_slotcount = 1; //manage when TX processing is triggered
  hw_cfg[0].iq_txshift = 4;//shift
  hw_cfg[0].iq_rxrescale = 15;//rescale iqs

  if(hw_cfg[0].tx_subdev!=NULL){
    LOG_I(HW, "hw_cfg[0].tx_subdev == %s\n", hw_cfg[0].tx_subdev);
    tx_subdev = hw_cfg[0].tx_subdev;
    s->usrp->set_tx_subdev_spec(tx_subdev);
  }

  if(hw_cfg[0].rx_subdev!=NULL){
    LOG_I(HW, "hw_cfg[0].rx_subdev == %s\n", hw_cfg[0].rx_subdev);
    rx_subdev = hw_cfg[0].rx_subdev;
    s->usrp->set_rx_subdev_spec(rx_subdev);
  }

  for(int i=0; i<((int) s->usrp->get_rx_num_channels()); i++) {
    openair0_config_t *cfg = &hw_cfg[0];
  ...
  LOG_D(HW, "hw_cfg[0].tx_num_channels == %d\n", hw_cfg[0].tx_num_channels);

  for(int i=0; i<((int) s->usrp->get_tx_num_channels()); i++) {
    ::uhd::gain_range_t gain_range_tx = s->usrp->get_tx_gain_range(i);

    if (i<hw_cfg[0].tx_num_channels) {
      s->usrp->set_tx_rate(hw_cfg[0].sample_rate,i+choffset);
      uhd::tune_request_t tx_tune_req(hw_cfg[0].tx_freq[i],
                                      hw_cfg[0].tune_offset);
      s->usrp->set_tx_freq(tx_tune_req, i+choffset);
      s->usrp->set_tx_gain(gain_range_tx.stop()-hw_cfg[0].tx_gain[i],i+choffset);
      LOG_I(HW,"USRP TX_GAIN:%3.2lf gain_range:%3.2lf tx_gain:%3.2lf\n", gain_range_tx.stop()-hw_cfg[0].tx_gain[i], gain_range_tx.stop(), hw_cfg[0].tx_gain[i]);
    }
  }
  ...
  for (int i = 0; i<hw_cfg[0].tx_num_channels; i++)
  ...
  if (device->type != USRP_N300_DEV) {
    for(int i=0; i<((int) s->usrp->get_tx_num_channels()) && i<hw_cfg[0].tx_num_channels; i++)
      s->usrp->set_tx_bandwidth(hw_cfg[0].tx_bw,i+choffset);

    for(int i=0; i<((int) s->usrp->get_rx_num_channels()) && i<hw_cfg[0].rx_num_channels; i++)
      s->usrp->set_rx_bandwidth(hw_cfg[0].rx_bw,i+choffset);
  }
  ...
  s->sample_rate = hw_cfg[0].sample_rate;
  ...
  if (recPlay != NULL) { // record mode
    recPlay->maxSizeBytes=hw_cfg[0].recplay_conf->u_sf_max *
                            (sizeof(iqrec_t)+BELL_LABS_IQ_BYTES_PER_SF);
